    ...
    def train(self, max_epochs: int):
        for epoch in range (self.epochs_run, max_epochs+1):
            ...
            if epoch % self.save_period == 0:
                self.train_node.save(self._make_snapshot(epoch)) # [MLC] Save model data
...
def main (save_period: int, total_epochs: int, batch_size: int, snapshot_path: str, train_node: MLC_BASE.TRAIN):
...
if __name__ == "__main__":
    ...
    parser.add_argument ('--starting_epoch', default=1, type=int, help='Input the start epoch for the MLC module. If epoch information exists in the snapshot file, that information will take precedence. (default: 1)')
    ...
    communicator, train_node, _ = MLC_BASE.init_MLC(args) # [MLC] Initialize the MLC environment.

    main(args.save_period, args.total_epochs, args.batch_size, args.snapshot_path, train_node)

    MLC_BASE.destroy_MLC() # [MLC] Destroy the MLC environment.